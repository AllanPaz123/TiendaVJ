...
<section class="grid py-4 px-4 my-4">
    <div class="row">
        <div class="col-12 offset-m-1 col-m-10 offset-l-3 col-l-6">
            <form class="row"
                action="index.php?page=Videojuegos-Inventario&mode={{mode}}&inventarioid={{inventarioid}}"
                method="post">
                <div class="row">
                    <label for="inventarioid" class="col-12 col-m-4">ID</label>
                    <input readonly type="text" class="col-12 col-m-8" name="inventarioid" id="inventarioid"
                        value="{{inventarioid}}" />
                    <input type="hidden" name="xsrToken" value="{{xsrToken}}" />
                </div>

                <div class="row">
                    <label for="videojuegocod" class="col-12 col-m-4">Código Videojuego</label>
                    <input type="number" class="col-12 col-m-8" name="videojuegocod" id="videojuegocod"
                        value="{{videojuegocod}}" {{readonly}} />
                ...
                <div class="row">
                    ...
                    <input type="text" class="col-12 col-m-8" name="ubicacion" id="ubicacion" value="{{ubicacion}}"
                        {{readonly}} />
                    {{if error_ubicacion}}
                ...
                <div class="row flex-end">
                    ...
                    &nbsp;
                    {{if showAction}}<button class="primary">Confirmar</button>{{endif showAction}}
                </div>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        document.getElementById("btnCancel").addEventListener("click", e => {
            e.preventDefault();
            e.stopPropagation();
            window.location.assign("index.php?page=Videojuegos-Inventarios");
        });
    });
</script>
